...
open class SegmentedControl: UIControl {
    ...
    var segmentSelectedColor: UIColor = UIColor.red
    ...
    open func setSelected(_ selected: Bool, forSegmentAtIndex index: Int) {
        ...
        UIView.animate(withDuration: animationDuration,
            ...
            completion: { _ in
                ...
                previousButton?.titleLabel?.textColor = (previousButton?.isSelected ?? true) ? self.segmentSelectedColor : self.segmentNormalColor
                currentButton?.titleLabel?.textColor = (currentButton?.isSelected ?? true) ? self.segmentSelectedColor : self.segmentNormalColor
    ...
    fileprivate func addButtonForSegment(_ segment: Int) {
        ...
        if segmentsContainerView == nil {
            ...
            self.sendSubviewToBack(segmentsContainerView!)
    ...
    fileprivate func setButtonImages(at position: Int) {
        ...
        if case .image(let image) = items[position].type {
            ...
            button?.setImage(image.normal, for: UIControl.State())
    ...
    fileprivate func configureButtonForSegment(_ segment: Int) {
        ...
        if let title = titleForSegmentAtIndex(segment) {
            ...
            setTitleColor(titleColorForButtonState(UIControl.State()), forState: UIControl.State())
    ...
    fileprivate func setTitleColor(_ color: UIColor, forState state: UIControl.State) {
        for button in segmentsButtons {
            button.setTitleColor(color, for: state)
        }
    }
    
    fileprivate func titleColorForButtonState(_ state: UIControl.State) -> UIColor {
        switch state {
        case UIControl.State():		return segmentNormalColor
        case UIControl.State.disabled:	return segmentDisabledColor
            case UIControl.State.selected:    return segmentSelectedColor
