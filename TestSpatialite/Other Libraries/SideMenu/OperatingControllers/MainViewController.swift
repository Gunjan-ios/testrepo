...
class MainViewController: LGSideMenuController {
    ...
    var myVTVNavigarionController:UINavigationController?
...
//    }
    ...
    func setup(type: UInt) {
        ...
        if (self.storyboard != nil) {
            ...
            leftViewWidth = SIDE_PANEL_WIDTH;
    ...
    override func leftViewWillLayoutSubviews(with size: CGSize) {
        ...
        if !isLeftViewStatusBarHidden {
            ...
            leftView?.frame = CGRect(x: 0, y: STATUS_BAR_HEIGHT, width: Int(size.width), height: (Int(size.height) - STATUS_BAR_HEIGHT))
        }
        
        //(self.leftViewController as! UITableViewController).tableView.setContentOffset(.zero, animated: false)
        (self.leftViewController as! LeftViewController).layerTableView.setContentOffset(.zero, animated: false)
    }
    ...
    override func rightViewWillLayoutSubviews(with size: CGSize) {
        ...
        if (!isRightViewStatusBarHidden ||
            (rightViewAlwaysVisibleOptions.contains(.onPadLandscape) &&
                UI_USER_INTERFACE_IDIOM() == .pad && UIApplication.shared.statusBarOrientation.isLandscape)) {
