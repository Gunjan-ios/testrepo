...
@IBDesignable class DPOTPView: UIView {
    ...
    @IBInspectable var tintColorTextField: UIColor = UIColor.blue
                ...
                if isCircleTextField {
                    ...
                    if i == 1 {
                        ...
                        if #available(iOS 11.0, *) {
                            ...
                            rectShape.bounds = textField.frame
                            rectShape.position = textField.center
                            rectShape.path = UIBezierPath(roundedRect: textField.bounds, byRoundingCorners: [.topLeft , .bottomLeft], cornerRadii: CGSize(width: 10, height: 10)).cgPath
                            //textField.layer.mask = rectShape
                            
                            textField.layer.addSublayer(rectShape)
                            textField.borderStyle = .none
                        }
                        ...
                        if #available(iOS 11.0, *) {
                            ...
                            rectShape.bounds = textField.frame
                            rectShape.position = textField.center
                            rectShape.path = UIBezierPath(roundedRect: textField.bounds,    byRoundingCorners: [.topRight , .bottomRight], cornerRadii: CGSize(width: 10, height: 10)).cgPath
                            //textField.layer.mask = rectShape
                            textField.layer.addSublayer(rectShape)
                            textField.borderStyle = .none
                        }
...
//            textField.frame = CGRect(x:(CGFloat(i-1) * sizeTextField) + (CGFloat(i) * spacing/2) + (CGFloat(i-1) * spacing/2)  , y: (self.bounds.height - sizeTextField)/2 , width: sizeTextField, height: sizeTextField)
             textField.frame = CGRect(x:(CGFloat(i-1) * sizeTextField) + (CGFloat(i) * spacing/2) + (CGFloat(i-1) * spacing/2)  , y: 0 , width: sizeTextField, height: self.bounds.height)
...
//        return super.canPerformAction(action, withSender: sender)
    ...
    override func textRect(forBounds bounds: CGRect) -> CGRect {
        if dpOTPView != nil {
            ...
            return bounds.inset(by: UIEdgeInsets.zero)
        }
    }
