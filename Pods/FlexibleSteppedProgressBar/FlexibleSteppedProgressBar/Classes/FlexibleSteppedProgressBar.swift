...
//            animationRendering = true
    ...
    open var visitedStepTextColor: UIColor!
    ...
    fileprivate var _textLayers = [UIImageView]()
    ...
    fileprivate func renderTextIndexes() {
        ...
        for i in 0...(numberOfPoints - 1) {
            ...
            textLayer.image = UIImage(named: "buttonSetting")
            ...
            if i == currentIndex || i == completedTillIndex {
                //textLayer.foregroundColor = visitedStepTextColor.cgColor
            } else {
                //textLayer.foregroundColor = centerLayerTextColor?.cgColor
               // textLayer.foregroundColor = stepTextColor.cgColor
            }
            
            if let image = self.delegate?.progressBar?(self, imageAtIndex: i, position: FlexibleSteppedProgressBarTextLocation.center) {
                textLayer.image = image.image
            } else {
//                textLayer.string = "\(i)"
                textLayer.image = UIImage(named: "buttonSetting")
            }
    ...
    /**
     ...
     - returns: The text layer
     */
    fileprivate func _textLayer(atIndex index: Int) -> UIImageView {
        
        var textLayer: UIImageView!
        
        if self._textLayers.count > index {
            
            let _textLayer = self._textLayers[index]
            
            if (_textLayer.image != nil) {
                textLayer = _textLayer
            } else {
                textLayer = UIImageView()
                //self._textLayers[index] = textLayer
                self._textLayers.append(textLayer)
            }
            //self.layer.addSublayer(textLayer)
            ...
            let textLayer = UIImageView(image: UIImage(named: "buttonSetting"))
            self._textLayers.append(textLayer)
//            self._textLayers[index] = textLayer
